...
public sealed partial class SubFire_CreateFire_Patch : NitroxPatch, IDynamicPatch
{
    ...
    {
        // Если командует сервер — пропускаем без проверок на владение
        if (NitroxClient.Communication.Packets.Processors.CyclopsFireCreatedProcessor.IsSpawningFromNetwork)
        {
            __state = true;
            return true; // Разрешаем оригинальному методу игры создать огонь!
        }

        bool isLocalPilotInsideThisCyclops =
            Player.main != null &&
            Player.main.isPiloting &&
            Player.main.currentSub == __instance.subRoot;

        __state = isLocalPilotInsideThisCyclops ||
                  (__instance.subRoot.TryGetIdOrWarn(out NitroxId id) &&
                   Resolve<SimulationOwnership>().HasAnyLockType(id));

        // Never block local fire creation. We only gate NETWORK broadcast in Postfix via __state.
        return true;
    }
