...
public sealed partial class CyclopsMotorModeButton_OnClick_Patch : NitroxPatch, IDynamicPatch
{
    ...
    {
        // Always re-broadcast Cyclops metadata after clicking the mode button.
        // Even when game-side guard paths toggle __state, remote clients still need the latest mode.
        if (__instance.subRoot.TryGetIdOrWarn(out NitroxId id))
        {
            // Перед отправкой данных убедимся, что мы актуализировали состояние
            CyclopsMotorMode motorMode = __instance.subRoot.GetComponentInChildren<CyclopsMotorMode>(true);
            if (motorMode != null)
            {
                // Тут можно добавить логику записи температуры в метаданные, 
                // если в Nitrox есть такое поле. Но для начала просто пнем обновление:
