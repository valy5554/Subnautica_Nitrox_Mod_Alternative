...
public class NitroxCyclops : MonoBehaviour
{
    ...
    // Remote override of CyclopsNoiseManager.GetNoisePercent for non-authoritative clients.
    // This is fed by network snapshots when someone else pilots this Cyclops.
    public float RemoteNoisePercent { get; private set; }
    public float RemoteNoiseUpdatedAt { get; private set; }

    public void SetRemoteNoisePercent(float percent)
    {
        RemoteNoisePercent = Mathf.Clamp01(percent);
        RemoteNoiseUpdatedAt = Time.unscaledTime;
    }
