    ...
    public class SimulationOwnership
    {
        ...
        public bool TreatVehicleEntity(NitroxId entityId, bool isLocalPlayerNewOwner, SimulationLockType simulationLockType)
        {
            ...
            if (isLocalPlayerNewOwner)
            {
                ...
                Log.Debug($"[SIM] Vehicle {entityId} -> LOCAL OWNER (broadcaster). lock={simulationLockType}");
            }
            else
            {
                if (!movementReplicator)
                {
                    ...
                    Log.Debug($"[SIM] Vehicle {entityId} -> REMOTE (replicator added). lock={simulationLockType}");
                }
                else
                {
                    Log.Debug($"[SIM] Vehicle {entityId} -> REMOTE (replicator already exists). lock={simulationLockType}");
                }
